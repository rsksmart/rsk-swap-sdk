...
import { BlockchainConnection, type BridgeError } from '@rsksmart/bridges-core-sdk'
...
jest.mock('@rsksmart/bridges-core-sdk', () => {
  return {
    ...
    BridgeError: jest.requireActual<{ BridgeError: BridgeError }>('@rsksmart/bridges-core-sdk').BridgeError
  }
})
...
describe('executeSwap function should', () => {
  ...
  test('fail on unknown action', async () => {
    ...
    try {
      ...
      expect(e.details.cause).toBe('Unsupported swap action type: UNKNOWN')
    }
  })
})
